...
from src.util import (is_valid_keypoint_1, pair_dir, int_tuple, preprocess_image, is_point_on_left, get_angle)
from shapely.geometry import LineString, Point, MultiPoint
from shapely.ops import nearest_points
import argparse
from silhouette import  SilhouetteExtractor
from src.pytorch_openpose import torch_openpose
    ...
    for p in contour_string.coords:
        if p[0] > x_range[0] and p[0] < x_range[1] and p[1] > lhip[1]:
            if p[1] <  min_y:
                min_y = p[1]
                crotch = p
    return np.array(crotch).flatten('C')
...
def estimate_front_collar(contour_string, keypoints):
    ...
    if (lcollar is not None and lcollar.shape[0] == 1) and (rcollar is not None and  rcollar.shape[0] == 1):
        collar_0 = rcollar if lcollar[0,0] > rcollar[0,0] else lcollar
        collar_1 = lcollar if lcollar[0,0] > rcollar[0,0] else rcollar

        # initial collar point are streteched horizontally
        mid_h = (collar_0[0][1] + collar_1[0][1]) / 2
        collar_0[0][1] = collar_1[0][1] = mid_h
        collar_0[0][0] -= 20
        collar_1[0][0] += 20
        collar_line = LineString([collar_0[0], collar_1[0]])
        pts_intersection = collar_line.intersection(contour_string)
        a = Point(3,3)

        collar_0[0][0]  =  pts_intersection[0].x
        collar_1[0][0]  =  pts_intersection[1].x


        # collar_0[0][1] = pts_intersection.

            # modified because for slope for shouler, a little unsafe, but , if the contour is correct, it is safe
...
def estimate_front_shoulder_points(contour_string, keypoints):
    ...
    test = nearest_points(Point(p0), isect_points)
...
def draw_slice_data(img, contour, slices, LINE_THICKNESS = 2):
    ...
    for name, points in slices.items():
        if 'Aux_' not in name:
            ...
            cv.putText(img, name, int_tuple(points[0]), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,0))
    #
    for name, points in slices.items():
        if 'Aux_' in name:
...
def load_pair_info(pair_text_file_path):
    ...
    with open(pair_text_file_path) as f:
        for line in f.readlines():
            img_name_f, img_name_s, height, weight = line.split()
            height = float(height)
            weight = float(weight)
            pairs.append((img_name_f, img_name_s, height, weight))
    return pairs

def get_fit(weight, height):
    BMI = weight / (height ** 2)

def get_arm_type(img, sil, keypoints_s):
    clr_nect2hip = (255, 0, 0)
    thickness = 2
    clr_arm = (0, 255, 0)
    ...
    font_thickness = 2
    text_color = (0, 0, 0)
    font_coordinate = (10,100)
    str_arm_type = ["Arm Curved", "Arm_Very_Backward", "Arm_Very_Forward", "Arm_Normal", "Arm_Straight", "Arm_Slight_Backward"]
    type = 0
    # cv.imshow("src_image", img)

    lshoulder = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['LShoulder']][:2], np.int)
    lelbow = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['LElbow']][:2], np.int)
    lwrist = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['LWrist']][:2], np.int)
    neck = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['Neck']][:2], np.int)
    mid_hip = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['MidHip']][:2], np.int)

    cv.line(img, neck, mid_hip, clr_nect2hip, thickness)
    cv.line(img, lshoulder, lelbow, clr_arm, thickness)
    cv.line(img, lelbow, lwrist, clr_arm, thickness)

    #  here O is intersection point of neck_mid and shoulder_elbow
    ang_elbow_O_midhip = cv2.minAreaRect(np.array([neck, mid_hip, lshoulder, lelbow]))[2]
    ang_lshoulder_lelbow_lwrist = cv.minAreaRect(np.array([lelbow, lshoulder, lelbow, lwrist]))[2]
    dir_shoulder_neck = lshoulder - neck
    str_dis = str(dir_shoulder_neck[0]).format('.3f')
    str_ang = f'{ang_lshoulder_lelbow_lwrist:.2f}'
    if dir_shoulder_neck[0] > 1 and ang_lshoulder_lelbow_lwrist > 10:
        type = 0

    elif dir_shoulder_neck[0] > 7 :
        type = 1
    elif dir_shoulder_neck[0] < -3:
        type = 2
    else:
        type = 3
    cv.putText(img, f"{str_arm_type[type]}: ANG={str_ang}, DIS={str_dis}",
               font_coordinate, font,
               font_scale, text_color, font_thickness, cv.LINE_4)
    # cv2.putText(img, "Arm Curved", (100, 100), font, font_scale, text_color, font_thickness, cv.LINE_AA)

    # cv.imshow("Arm Type", img)
    # cv.imwrite(f"{str_arm_type[type]}.jpg", img)
    # cv.waitKey(0)

cnt = 0
def get_back_style(img_s, sil_s, keypoints_s, contour_s, landmark_s):

    clr_neck2hip = (255, 0, 0)
    thickness = 2
    clr_arm = (0, 255, 0)
    font = cv2.FONT_HERSHEY_SIMPLEX
    font_scale = 1
    ...
    str_back_type = ["Back Normal", "Back Forward Head", "Back Humpack + Forward Head", "Back Style S"]
    type = 0
    # cv.imshow("src_image", img)
    nose = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['Nose']][:2], np.int)
    neck = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['Neck']][:2], np.int)
    mid_hip = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['MidHip']][:2], np.int)

    cv.line(img_s, neck, mid_hip, clr_neck2hip, thickness)
    cv.line(img_s, neck, nose, clr_neck2hip, thickness)

    #  here O is intersection point of neck_mid and shoulder_elbow
    ang_nose_neck_midhip = get_angle(neck, nose, mid_hip)
    armscye = landmark_s['Armscye']
    aux_waist_underbust_0 = landmark_s['Aux_Waist_UnderBust_0']
    dis_armscye_waist = armscye[1][0] - aux_waist_underbust_0[1][0]
    str_dis = f'{dis_armscye_waist:.2f}'
    str_ang = f'{ang_nose_neck_midhip:.2f}'
    if ang_nose_neck_midhip < 135:
        type = 2
        if dis_armscye_waist > 10:
            type = 3
    elif ang_nose_neck_midhip < 140:
        type = 1
    else:
        type = 0

    cv.putText(img_s, f"{str_back_type[type]}: ANG={str_ang} DIS={str_dis}",
               int_tuple(aux_waist_underbust_0[1]), font,
               font_scale, text_color, font_thickness, cv.LINE_AA)
    # cv2.putText(img, "Arm Curved", (100, 100), font, font_scale, text_color, font_thickness, cv.LINE_AA)
    draw_slice_data(img_s, contour_s, landmark_s)


    cv.imshow("Back Style", img_s)
    # cv.imwrite(f"{str_back_type[type]}_{str_ang}.jpg", img_s)
    cv.waitKey(0)

def get_belly_type(img_s, sil_s, keypoints_s, contour_s, landmark_s):
    clr_nect2hip = (255, 0, 0)
    thickness = 2
    clr_arm = (0, 255, 0)
    font = cv2.FONT_HERSHEY_SIMPLEX
    ...
    text_color = (0, 0, 0)
    font_coordinate = (10, 50)
    str_back_type = ["Belly Normal", "Belly-Tenesmic stomach high", "Bend forward of abdomen", "Belly Portly"]
    type = 0
    # cv.imshow("src_image", img)
    # nose = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['Nose']][:2], np.int)
    # neck = np.array(keypoints_s[0][POSE_BODY_25_BODY_PART_IDXS['Neck']][:2], np.int)
    ...
    # cv.line(img_s, neck, mid_hip, clr_nect2hip, thickness)
    # cv.line(img_s, neck, nose, clr_nect2hip, thickness)

    #  here O is intersection point of neck_mid and shoulder_elbow
    # ang_nose_neck_midhip = get_angle(neck, nose, mid_hip)
    waist = landmark_s['Waist']
    aux_waist_underbust_0 = landmark_s['Aux_Waist_UnderBust_0']
    under_bust = landmark_s['UnderBust']
    bust = landmark_s['Bust']
    aux_hip_waist_0 = landmark_s['Aux_Hip_Waist_0']
    aux_crotch_hip_0 = landmark_s['Aux_Crotch_Hip_0']
    str_id = f'{bust[0][0]:.2f}'
    if waist[0][0] < bust[0][0] and waist[0][0]-5 < aux_waist_underbust_0[0][0] - 5:
        type = 1
    elif aux_waist_underbust_0[0][0] < bust[0][0] - 5 and aux_waist_underbust_0[0][0] < waist[0][0] - 5:
        type = 2
    elif aux_crotch_hip_0[0][0] < bust[0][0] and aux_hip_waist_0[0][0] < bust[0][0]:
        type = 3
    else:
        type = 0

    cv.putText(img_s, f"{str_back_type[type]}",
               int_tuple(aux_waist_underbust_0[0] - 50), font,
               font_scale, text_color, font_thickness, cv.LINE_AA)
    # cv2.putText(img, "Arm Curved", (100, 100), font, font_scale, text_color, font_thickness, cv.LINE_AA)
    draw_slice_data(img_s, contour_s, landmark_s)


    cv.imshow("Belly Type", img_s)
    # cv.imwrite(f"{str_back_type[type]}_{str_id}.jpg", img_s)
    cv.waitKey(0)
def get_body_type(img_s, sil_s, keypoints_s, contour_s, landmark_s):
    ...
    text_color = (0, 0, 0)
    font_coordinate = (10, 50)
    str_back_type = ["Body Normal", "Body Forward", "Body Backward", "Body Stooped"]
    type = 0

    neck = landmark_s['Neck']
    #  caution! the hair must be considered
    armscye = landmark_s['Armscye']
    collar = landmark_s['Collar']
    dis_neck_armscye = neck[1][0] - armscye[1][0]
    dis_collar_armscye = collar[1][0] - armscye[1][0]
    str_id = f'{dis_neck_armscye:.2f}'
    if dis_neck_armscye < -35 and dis_collar_armscye < -30:
        type = 3
    elif dis_neck_armscye <  -25 and dis_collar_armscye < -20:
        type = 1
    elif dis_neck_armscye > -5 and dis_collar_armscye > -3:
        type = 2
    else:
        type = 0
    ...
    # cv.imwrite(f"{str_back_type[type]}_{str_id}.jpg", img_s)
    cv.waitKey(0)
def get_hip_type(img_s, sil_s, keypoints_s, contour_s, landmark_s):
    ...
    text_color = (0, 0, 0)
    font_coordinate = (10, 50)
    str_hip_type = ["Hip Normal", "Prominent Seat", "Drop Seat", "Flat Seat"]
    ...
    hip = landmark_s['Hip']
    aux_crotch_hip_0 = landmark_s['Aux_Crotch_Hip_0']
    crotch = landmark_s['Crotch']
    dis_hip_crotch = hip[1, 0] - crotch[1,0]
    dis_hip_aux_crotch = hip[1, 0] - aux_crotch_hip_0[1, 0]
    str_id = f'{dis_hip_crotch:.2f}'
    if dis_hip_aux_crotch < -5:
        type = 2
    elif dis_hip_crotch > 15:
        type = 1
    elif dis_hip_crotch < 5:
        type = 3
    else:
        type = 0
    ...
    waist = landmark_s["Waist"]
    armscye = landmark_s["Armscye"]
    dis_waist_armscye = waist[1, 0] - armscye[1, 0]
    dis_waist_hip = waist[1, 0] - hip[1, 0]
    if dis_waist_armscye < -20 and dis_waist_hip > -10:
        waist_type = 2
    elif dis_waist_armscye < -10:
        waist_type = 1
    else:
        waist_type = 0


    cv.putText(img_s, f"{str_hip_type[type]}",int_tuple(hip[1]), font, font_scale, text_color, font_thickness, cv.LINE_AA)
    cv.putText(img_s, f"{str_waist_type[waist_type]}", int_tuple(waist[1]), font, font_scale, text_color, font_thickness,
               cv.LINE_AA)
    # cv2.putText(img, "Arm Curved", (100, 100), font, font_scale, text_color, font_thickness, cv.LINE_AA)
    draw_slice_data(img_s, contour_s, landmark_s)


    cv.imshow("Hip_Waist Type", img_s)
    # cv.imwrite(f"{str_hip_type[type]}_{str_id}.jpg", img_s)
    cv.waitKey(0)
def rearrange_in_x_order(landmark_s):
    for name, points in landmark_s.items():
        print(points)
        if name == "Height":
            continue
    ...
    return landmark_s
def get_shoulder_type(img_f, sil_f, keypoints_f, contour_f, landmark_f, img_s, sil_s, keypoints_s, contour_s, landmark_s):
    ...
    str_shoulder_bulge_blade_type = ['Blade Normal', 'Bulge Blade', 'Bulge Top Blade']
    str_shouler_blade_inner_type = ['Shoulder Blade Normal', 'Shoulder Blade Innder']
    shoulder_bulge_blade_type = 0
    shoulder_blade_inner_type = 0
    def get_slope_loc(degree):
        if degree < 8:
            return 0
        elif degree < 12.5:
            return 1
            ...
            return 6
    shoulder_f = landmark_f['Shoulder']
    collar_f = landmark_f['Collar']
    def point_checker(p):
        if p[1] < shoulder_f[0, 1] and p[1] > collar_f[0, 1]:
            return True
        else:
            return False

    chains = extract_contour_chains(contour_f, point_checker)
    ...
    local_maximas = []
    for i in range(half_win_size, len(chains[0]) - half_win_size -1):

        pt_cur = chains[0][i]
        pts_neighbor = chains[0][i-half_win_size:i+half_win_size + 1]
        argmin = np.argmin(pts_neighbor, axis = 0)
        if argmin[1] == i:
            local_maximas.append(pt_cur)

    if len(local_maximas) == 0:
        shoulder_blade_inner_type = 0
    elif local_maximas[0][0][0] - shoulder_f[0][0] < 15:
        shoulder_blade_inner_type = 0
    else:
        shoulder_blade_inner_type = 1

    r_slope = math.degrees(math.atan2(-collar_f[0,1] + shoulder_f[0, 1], collar_f[0,0] - shoulder_f[0, 0]))
    l_slope = math.degrees(math.atan2(-collar_f[1,1] + shoulder_f[1, 1], -collar_f[1,0] + shoulder_f[1, 0]))
    r_type = get_slope_loc(r_slope)
    l_type = get_slope_loc(l_slope)

    r_collar_shoulder_line = LineString([collar_f[0], shoulder_f[0]])

    aux_armscye_s = landmark_s['Aux_Armscye_Shoulder_0']
    armscye_s = landmark_s['Armscye']
    dis_auxarmscye_armscye = aux_armscye_s[1, 0] - armscye_s[1, 0]
    if dis_auxarmscye_armscye > 6:
        shoulder_bulge_blade_type = 2
    elif dis_auxarmscye_armscye > 3:
        shoulder_bulge_blade_type = 1
    else:
        shoulder_bulge_blade_type = 0

    cv.putText(img_s, f"{str_shoulder_bulge_blade_type[shoulder_bulge_blade_type]}",
               int_tuple(armscye_s[1]), font,
               font_scale, text_color, font_thickness, cv.LINE_AA)
    cv.putText(img_f, f"{str_shoulder_bulge_blade_type[shoulder_bulge_blade_type]}",
               int_tuple(shoulder_f[0] - [50,30]), font,
               font_scale, text_color, font_thickness, cv.LINE_AA)
    cv.putText(img_f, f"{str_slope_type[r_type]}",
               int_tuple(shoulder_f[0]), font,
               font_scale, text_color, font_thickness, cv.LINE_AA)
    cv.putText(img_f, f"{str_slope_type[l_type]}",
               int_tuple(shoulder_f[1]), font,
               font_scale, text_color, font_thickness, cv.LINE_AA)
    # cv2.putText(img, "Arm Curved", (100, 100), font, font_scale, text_color, font_thickness, cv.LINE_AA)
    draw_slice_data(img_f, contour_f, landmark_f)
    draw_slice_data(img_s, contour_s, landmark_s)

    cv.imshow("Shoulder Slope", img_f)
    cv.waitKey(0)
    cv.imshow("Shoulder Slope", img_s)
    cv.waitKey(0)
def get_chest_type(img_f, sil_f, keypoints_f, contour_f, landmark_f, img_s, sil_s, keypoints_s, contour_s, landmark_s):
    ...
    shoulder = landmark_f['Shoulder']
    collar = landmark_f['Collar']
    str_chest_type = ['Normal Chest', 'Chest Out', 'Well built Chest']
    chest_type = 0
    armscye_f = landmark_f['Armscye']
    waist = landmark_f['Waist']
    armscye_width = armscye_f[1, 0] - armscye_f[0, 0]
    waist_width = waist[1, 0] - waist[0, 0]
    dis_armscye_waist = armscye_width - waist_width
    armscye_s = landmark_s['Armscye']
    bust_s = landmark_s['Bust']
    dis_armscye_bust_s = armscye_s[0, 0] - bust_s[0, 0]

    if dis_armscye_waist > 20:
        chest_type = 2
    elif dis_armscye_bust_s < 0:
        chest_type = 1
    else:
        chest_type = 0
    ...
    if chest_type ==  1:
        cv.putText(img_s, f"{str_chest_type[chest_type]}",
                   int_tuple(armscye_s[0]), font,
                   font_scale, text_color, font_thickness, cv.LINE_AA)
        # cv.imwrite(f"{str_chest_type[chest_type]}{dis_armscye_bust_s}.jpg", img_s)
        cv.imshow("Shoulder Slope", img_f)

        cv.waitKey(0)
    else:
        cv.putText(img_f, f"{str_chest_type[chest_type]}",
                   int_tuple(armscye_f[0]), font,
                   font_scale, text_color, font_thickness, cv.LINE_AA)
        # cv.imwrite(f"{str_chest_type[chest_type]}{dis_armscye_waist}.jpg", img_f)
        cv.imshow("Shoulder Slope", img_f)
...
if __name__ == '__main__':
    ...
    IMG_DIR = '../data/images' + '/'
    SIL_DIR = '../data/silhouette' + '/'
    POSE_DIR = '../data/pose' + '/'
    pair_path = '../data/pair.txt'
    OUT_DIR = '../data/measurement' + '/'
    ...
    body_estimation = torch_openpose.torch_openpose('body_25')

    sil_extractor = SilhouetteExtractor(use_mobile_model = False, use_gpu=False)



    # img_s = cv.imread('../../specification/arm/arm_curved.jpg')
    # sil_s = load_silhouette(f'{SIL_DIR}{path_f.name}', img_f)
    # keypoints_s = np.load('../data/pose/arm_curved_keypoints.npy')
    # get_arm_type(img_s,sil_s,  keypoints_s)


    pairs = load_pair_info(pair_text_file_path=pair_path)
    for img_name_f, img_name_s, height, weight in pairs:
        path_f = Path(f'{IMG_DIR}{img_name_f}')
        path_s = Path(f'{IMG_DIR}{img_name_s}')

        img_f = cv.imread(str(path_f))  # B,G,R order
        img_s = cv.imread(str(path_s))
        if img_f is None or img_s is None:
            ValueError('Image is not found')
        img_f = preprocess_image(img_f)
        img_s = preprocess_image(img_s)
        keypoints_f = body_estimation(img_f)
        keypoints_s = body_estimation(img_s)
        keypoints_f = np.array(keypoints_f)
        keypoints_s = np.array(keypoints_s)
        img_debug = img_f.copy()
        sil_deeplab_f, sil_refined_f = sil_extractor.extract_silhouette(img_f, True, keypoints_f, img_debug=img_debug)
        sil_deeplab_s, sil_refined_s = sil_extractor.extract_silhouette(img_s, False, keypoints_s, img_debug=None)

        data, final_viz = calc_body_landmarks_util(img_f, img_s, sil_refined_f, sil_refined_s, keypoints_f, keypoints_s, height = height)

        ordered_landmark_s = rearrange_in_x_order(data["landmark_segment_s"])
        ordered_landmark_f = rearrange_in_x_order(data["landmark_segment_f"])
        get_hip_type(img_s, sil_refined_s, keypoints_s,data['contour_s'], ordered_landmark_s)
        get_chest_type(img_f, sil_refined_f, keypoints_f, data['contour_f'], ordered_landmark_f, img_s, sil_refined_s, keypoints_s, data['contour_s'], ordered_landmark_s)
        get_shoulder_type(img_f, sil_refined_f, keypoints_f, data['contour_f'], ordered_landmark_f, img_s, sil_refined_s, keypoints_s, data['contour_s'], ordered_landmark_s)
        get_arm_type(img_s, sil_refined_s, keypoints_s)
        get_back_style(img_s, sil_refined_s, keypoints_s,data['contour_s'], ordered_landmark_s)
        get_body_type(img_s, sil_refined_s, keypoints_s,data['contour_s'], ordered_landmark_s)
        get_belly_type(img_s, sil_refined_s, keypoints_s, data['contour_s'], ordered_landmark_s)


        cv.imwrite(f'{OUT_DIR}/{path_f.stem}_f.jpg', img_f)
        cv.imwrite(f'{OUT_DIR}/{path_f.stem}_s.jpg', img_s)
