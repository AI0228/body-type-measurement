...
if __name__ == '__main__':
    ...
    for img_name_f, img_name_s, height, weight in pairs:
        ...
        keypoints_f = np.load(f'{POSE_DIR}/{path_f.stem}_keypoints.npy')
        sil_f = load_silhouette(f'{SIL_DIR}{path_f.name}', img_f)

        img_s = cv.imread(str(path_s))
        img_s = preprocess_image(img_s)
        keypoints_s = np.load(f'{POSE_DIR}/{path_s.stem}_keypoints.npy')
        sil_s = load_silhouette(f'{SIL_DIR}{path_s.name}', img_s)

        lknee = keypoints_f[0][POSE_BODY_25_BODY_PART_IDXS['LKnee']][:2]
        lankle = keypoints_f[0][POSE_BODY_25_BODY_PART_IDXS['LAnkle']][:2]
        lheel = keypoints_f[0][POSE_BODY_25_BODY_PART_IDXS['LHeel']][:2]
        rheel = keypoints_f[0][POSE_BODY_25_BODY_PART_IDXS['RHeel']][:2]
        ltoe = keypoints_f[0][POSE_BODY_25_BODY_PART_IDXS['LBigToe']][:2]
        rtoe = keypoints_f[0][POSE_BODY_25_BODY_PART_IDXS['RBigToe']][:2]
        ...
        get_hip_type(img_s, sil_s, keypoints_s,data['contour_s'], ordered_landmark_s)
        get_chest_type(img_f, sil_f, keypoints_f, data['contour_f'], ordered_landmark_f, img_s, sil_s, keypoints_s, data['contour_s'], ordered_landmark_s)
        get_shoulder_type(img_f, sil_f, keypoints_f, data['contour_f'], ordered_landmark_f, img_s, sil_s, keypoints_s, data['contour_s'], ordered_landmark_s)
        get_arm_type(img_s, sil_s, keypoints_s)
        get_back_style(img_s, sil_s, keypoints_s,data['contour_s'], ordered_landmark_s)
        get_body_type(img_s, sil_s, keypoints_s,data['contour_s'], ordered_landmark_s)
        get_belly_type(img_s, sil_s, keypoints_s, data['contour_s'], ordered_landmark_s)
